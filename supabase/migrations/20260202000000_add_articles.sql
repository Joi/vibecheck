...
-- Fix RLS for tool_mentions and tool_communities
DO $$
BEGIN
    -- tool_mentions insert policy
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'tool_mentions' AND policyname = 'Allow public insert to tool_mentions') THEN
        CREATE POLICY "Allow public insert to tool_mentions" ON tool_mentions
            FOR INSERT TO anon WITH CHECK (true);
    END IF;
    
    -- tool_communities insert policy  
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'tool_communities' AND policyname = 'Allow public insert to tool_communities') THEN
        CREATE POLICY "Allow public insert to tool_communities" ON tool_communities
            FOR INSERT TO anon WITH CHECK (true);
    END IF;
END $$;
