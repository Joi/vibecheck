...
from .database import ArticlesDB, CommunitiesDB, ToolsDB, get_admin_client
from .web import get_templates
...
def create_session_token(timestamp: int) -> str:
    ...
    signature = hmac.new(SECRET_KEY.encode(), message.encode(), hashlib.sha256).hexdigest()[:32]
...
async def admin_tool_save(
    ...
    client = get_admin_client()  # Use service role key for mutations
...
async def admin_tool_delete(
    ...
    tool = tools_db.get_tool(slug)
    if tool:
        client = get_admin_client()  # Use service role key for delete
